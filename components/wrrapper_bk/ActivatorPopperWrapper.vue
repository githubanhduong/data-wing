...
defineExpose({
  openPopper: () => popperState.value = true,
...
<template>
  <base-wrapper :label="label" :error-msg="errorMsg" v-click-outside="handleClickOutside" :data-muid="muid"
    ref="rootElement">
    <template #main_el>
      <slot name="activator" :class="['dw-activator', activatorClass]" @click="togglePopper"></slot>
      <teleport to="div.overlay-container">
        <slot name="popper" :class="['dw-popper', popperClass, { show: popperState }]" :muid="muid"></slot>
      </teleport>
    </template>
  </base-wrapper>
</template>