...
import type { PmInvoice } from '@/common/datawings-payment';

const props = defineProps<{
	index: number;
	modelValue: PmInvoice;
...
const emits = defineEmits<{
	(e: 'update:pmData', index: number, data: PmInvoice): void,
...
<template>
	<div class="mt-4 dynamic_form pm-credi-card linebt">
		<MessageText tag="h4" class="fw-bold" code="se.form.payment.type.invoice" text=".Invoice">
			<template #prepend>{{ index + 1 }}. </template>
			<!-- <template #default><i class="fas fa-credit-card ms-2"></i></template> -->
		</MessageText>
