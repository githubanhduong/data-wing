...
;(gj.widget = function () {
    ...
    return (
      ...
      $.isArray(d.dataSource)
        ...
        ? ((b = {
          ...
          (this.xhr = $.ajax(b).done(this.createDoneHandler()).fail(this.createErrorHandler())))
      ...
      if (d)
        for (var e = 0; e < d.length; e++)
          d[e].widgetId === b &&
            (d.splice(e, 1),
            (c = !0),
            0 === d.length && ($(document).off(a), delete gj.documentManager.events[a]))
  ...
  (gj.picker.widget.prototype.open = function (a) {
    ...
    return (
      ...
      b.modal ? gj.core.center(c) : (gj.core.setChildPosition(this[0], c[0]), this.focus()),
  ...
  (gj.dialog.methods = {
    ...
    resizable: function (a) {
      ...
      a.append(
        ...
        a.append(
          $('<div class="gj-resizable-handle gj-resizable-ne"></div>').draggable(
            $.extend(!0, {}, b)
          )
        ),
        a.append(
          $('<div class="gj-resizable-handle gj-resizable-nw"></div>').draggable(
            $.extend(!0, {}, b)
          )
        ),
        a.append(
          $('<div class="gj-resizable-handle gj-resizable-sw"></div>').draggable(
            $.extend(!0, {}, b)
          )
        ),
        a.append(
          $('<div class="gj-resizable-handle gj-resizable-se"></div>').draggable(
            $.extend(!0, {}, b)
          )
        )
    },
    ...
    open: function (a, b) {
      ...
      return (
        ...
        c.length &&
          c.outerHeight() &&
          a.children('div[data-role="body"]').css('margin-bottom', c.outerHeight()),
  ...
  (gj.grid.methods = {
    ...
    startLoading: function (a) {
      ...
      gj.grid.methods.stopLoading(a),
        ...
        0 !== a.outerHeight() &&
          ...
          (c = $('<div data-role="loading-cover" />').addClass(h.style.loadingCover).css({
            width: e,
            height: f,
            top: g
          })),
          (d = $(
            '<div data-role="loading-text">' + gj.grid.messages[h.locale].Loading + '</div>'
    ...
    renderRow: function (a, b, c, d) {
      ...
      for (
        h = a.data(),
          b && 0 !== b.length
            ? ((i = 'update'),
              b.removeClass(h.style.content.rowSelected).removeAttr('data-selected').off('click'))
    ...
    removeRow: function (a, b) {
      ...
      return (
        ...
        $.isArray(d.dataSource) &&
          ((c = gj.grid.methods.getRecVPosById(a, b)), d.dataSource.splice(c, 1)),
        a.reload(),
        a
      )
    },
        ...
        if (c.editor) {
          if (
            ...
            if ('function' == typeof c.editor)
              c.editor(f, h, d), (g = f.find('input, select, textarea').first())
            else if (
              ...
              'dropdown' === c.type && gj.dropdown
                ...
                : ((g = $('<input type="text" value="' + h + '" class="gj-width-full"/>')),
                  'materialdesign' === j.uiLibrary &&
                    g.addClass('gj-textbox-md').css('font-size', a.css('font-size')),
                  f.append(g))
        ...
        : a.on('cellDataBound', function (b, c, e, f, g) {
            f.editor &&
              ('editOnly' === f.mode
                ...
                : c
                    .parent('td')
                    .on('dblclick' === d.inlineEditing.mode ? 'dblclick' : 'click', function () {
                      gj.grid.plugins.inlineEditing.private.editMode(a, c.parent(), f, g)
                    }))
          })
  }),
        ...
        if (((d = a.data()), d.pager))
          for (
            d.params[d.paramNames.page] || (d.params[d.paramNames.page] = 1),
              ...
              c = $('<th/>'),
              b.append(c),
              f = $('<div data-role="display" />').addClass(d.style.pager.panel).css({
                float: 'left'
              }),
          ...
          (a.on('initialized', function () {
            a.makeResponsive(),
              ...
              (a.resizeCheckIntervalId = setInterval(function () {
                var b = a.width()
                b !== a.oldWidth &&
                  gj.grid.plugins.responsiveDesign.events.resize(a, b, a.oldWidth),
                  (a.oldWidth = b)
          ...
          (d = $(e[b])),
            void 0 !== c
              ? d
                  ...
                  .on(
                    'mousedown',
                    gj.grid.plugins.rowReorder.private.createRowMouseDownHandler(a, d)
                  )
              : d.on(
                  'mousedown',
                  gj.grid.plugins.rowReorder.private.createRowMouseDownHandler(a, d)
        ...
        if (g.length) {
          for (b = 0; b < f.length; b++)
            gj.grid.plugins.export.public.isColumnApplicable(f[b]) &&
              (d += '"' + (f[b].title || f[b].field).replace(/<[^>]+>/g, ' ') + '",')
  ...
  (gj.tree.methods = {
    ...
    appendNode: function (a, b, c, d, e) {
      ...
      if (
        ...
        ($expander = $('<span data-role="expander" data-mode="close"></span>').addClass(
          k.style.expander
        )),
        ...
        (disabled =
          void 0 !== c[k.disabledField] && 'true' === c[k.disabledField].toString().toLowerCase()),
        k.indentation &&
          $wrapper.append(
            '<span data-role="spacer" style="width: ' + k.indentation * (d - 1) + 'px;"></span>'
          ),
        disabled
        ...
        k.imageCssClassField && c[k.imageCssClassField]
          ? ((i = $(
              '<span data-role="image"><span class="' + c[k.imageCssClassField] + '"></span></span>'
            )),
          ...
          : k.imageHtmlField &&
            c[k.imageHtmlField] &&
            ((i = $('<span data-role="image">' + c[k.imageHtmlField] + '</span>')),
            i.insertBefore($display)),
    ...
    displayClickHandler: function (a) {
      return function (b) {
        ...
        'true' === d.attr('data-selected')
          ...
          : ('single' === a.data('selectionType') && gj.tree.methods.unselectAll(a),
    ...
    getNodeByText: function (a, b) {
      ...
      if (f && f.length)
        for (c = 0; c < f.length; c++) {
          if (
            ((d = $(f[c])), b === d.find('>[data-role="wrapper"]>[data-role="display"]').text())
          ) {
            e = d
            break
          }
        ...
        c && c.length
          ? ('li' === c[0].tagName.toLowerCase() &&
              (0 === c.children('ul').length &&
                (c.find('[data-role="expander"]').empty().append(g.icons.collapse),
      ...
      updateParentState: function (a, b) {
        ...
        ;(c = a.parent('ul').parent('li')),
          1 === c.length &&
            ...
            (e = a
              .siblings()
              .find('> [data-role="wrapper"] > span[data-role="checkbox"] input[type="checkbox"]')),
            (f = 'checked' === b),
            (g = 'unchecked' === b),
            (h = 'indeterminate'),
            $.each(e, function () {
    ...
    configure: function (a) {
      a.data('checkboxes') &&
        ...
        a.on('enable', function (a, b) {
          b.find('>[data-role="wrapper"]>[data-role="checkbox"] input[type="checkbox"]').prop(
            'disabled',
            !1
          )
        }),
    ...
    private: {
      nodeDataBound: function (a, b) {
        ...
        c.length &&
          ...
          d.on(
            'mousemove',
            gj.tree.plugins.dragAndDrop.private.createNodeMouseMoveHandler(a, b, d)
          ),
      ...
      createDoneHandler: function (a, b) {
        return function (c) {
          ...
          if (('string' == typeof c && JSON && (c = JSON.parse(c)), c && c.length)) {
            for (
              f = b.children('ul'),
                0 === f.length && ((f = $('<ul />').addClass(g.style.list)), b.append(f)),
                d = 0;
              d < c.length;
              d++
            )
  ...
  (gj.checkbox.methods = {
    ...
    initialize: function (a) {
      ...
      d.style.wrapperCssClass &&
        ((b = $(
          '<label class="' + d.style.wrapperCssClass + ' ' + d.style.iconsCssClass + '"></label>'
        )),
  ...
  (gj.dropdown.methods = {
      ...
      if ((a.data('records', b), a.empty(), f.empty(), b && b.length))
        if (
          ...
          a.prepend('<option value=""></option>'),
            (a[0].selectedIndex = 0),
            d.placeholder &&
              (h[0].innerHTML = '<span class="placeholder">' + d.placeholder + '</span>')
    ...
    close: function (a, b) {
      ...
      d.html(c.icons.dropdown),
        e &&
          ...
          gj.dropdown.methods.removeParentsScrollListener(e, c.parentScrollHandler),
        b.hide()
    },
    ...
    select: function (a, b) {
      ...
      return (
        ...
        g
          ? (e.addClass(c.style.active), (a[0].value = b), (f[0].innerHTML = g[c.textField]))
          : (c.placeholder &&
              (f[0].innerHTML = '<span class="placeholder">' + c.placeholder + '</span>'),
            (a[0].value = '')),
  ...
  (gj.datepicker.config = {
    base: {
      ...
      locale: 'en-us',
      icons: {
        rightIcon: '<i class="gj-icon calendar"></i>',
  ...
  (gj.datepicker.methods = {
    ...
    initialize: function (a, b) {
      ...
      0 === e.length
        ...
        b.keyboardNavigation &&
          $(document).on('keydown', gj.datepicker.methods.createKeyDownHandler(a, c, b))
    },
    createCalendar: function (a, b) {
      ...
      return (
        ...
        b.footer &&
          ...
          e.append(f),
          (g = $(
            '<button class="' + b.style.button + '">' + gj.core.messages[b.locale].ok + '</button>'
          )),
          g.on('click', function () {
            var c,
              d,
        ...
        $('body').append(h),
        b.modal &&
          (h.wrapAll('<div role="modal" class="' + b.style.modal + '"/>'), gj.core.center(h)),
        h
      )
    },
    ...
    createNavigation: function (a, b, c, d) {
      ...
      for (
        f = $('<div role="navigator" />'),
          ...
          f.append(
            $('<div role="period"></div>').on('click', gj.datepicker.methods.changePeriod(a, d))
          ),
          f.append(
            $('<div>' + d.icons.nextMonth + '</div>').on('click', gj.datepicker.methods.next(a, d))
          ),
          b.append(f),
          e = $('<tr role="week-days" />'),
    ...
    renderYear: function (a, b, c) {
      ...
      for (
        ...
        var $cell;
        for (var $row = $('<tr />'), f = 4 * e; f <= 4 * e + 3; f++)
            ...
            for (c = 0; c < a.disableDates.length; c++)
              a.disableDates[c] instanceof Date && a.disableDates[c].getTime() === b.getTime()
                ...
                : 'string' == typeof a.disableDates[c] &&
                  gj.core.parseDate(a.disableDates[c], a.format, a.locale).getTime() ===
                    b.getTime() &&
                  (d = !1)
    ...
    open: function (a, b) {
      ...
      switch (
        (a.val()
          ? a.value(a.val())
          : ((c = new Date()), d.attr('month', c.getMonth()), d.attr('year', c.getFullYear())),
        d.attr('type'))
      ) {
        case 'month':
      ...
      d.show(),
        ...
        b.modal
          ? gj.core.center(d)
          : (gj.core.setChildPosition(a[0], d[0]), document.activeElement !== a[0] && a.focus()),
  ...
  (gj.timepicker.methods = {
    ...
    createPicker: function (a) {
      var b,
        ...
        i = $('<div role="body" />'),
        j = $(
          '<button class="' + c.style.button + '">' + gj.core.messages[c.locale].ok + '</button>'
        ),
        k = $(
          '<button class="' +
            c.style.button +
      ...
      return (
        ...
        c.header &&
          ...
          'ampm' === c.mode &&
            (h.append(
              $('<span role="am">' + gj.core.messages[c.locale].am + '</span>').on(
                'click',
                function () {
            ...
            h.append('<br />'),
            h.append(
              $('<span role="pm">' + gj.core.messages[c.locale].pm + '</span>').on(
                'click',
                function () {
                  var b = gj.timepicker.methods.getHour(d)
                  d.attr('mode', 'pm'),
        ...
        $('body').append(d),
        c.modal &&
          (d.wrapAll('<div role="modal" class="' + c.style.modal + '"/>'), gj.core.center(d)),
        d
      )
    },
    getHour: function (a) {
  ...
  (gj.slider.methods = {
    ...
    initialize: function (a, b) {
      ...
      ;(a[0].style.display = 'none'),
        ...
        null == d &&
          ((d = document.createElement('div')), d.setAttribute('role', 'track'), c.appendChild(d)),
        gj.core.addClasses(d, b.style.track),
        (e = a[0].querySelector('[role="handle"]')),
        null == e &&
          ((e = document.createElement('div')), e.setAttribute('role', 'handle'), c.appendChild(e)),
